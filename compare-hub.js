...
// ─────────────────────────────────────────────────────────────────────────────
  ...
  (function() {
    ...
    // ── BROWSE_PHOTOS lives in /browse-photos.js ──────────────────────────
    // The curated headshot table moved out of this file so /ballot can have the
    // same faces without loading the Compare Hub: it is 715 portrait URLs, it
    // reads nothing and it touches no DOM, and the desk needs it. Loaded from
    // this file's old position in index.html, so the map is already on window by
    // the time anything below runs; the local binding keeps every reference in
    // this closure — and the `typeof BROWSE_PHOTOS !== 'undefined'` fallbacks
    // beside them — reading exactly what they always read.
    var BROWSE_PHOTOS = (function () {
      try { return window.BROWSE_PHOTOS || {}; } catch (e) { return {}; }
    })();
    ...
    // a real <a> to /me — the document that IS the file, which is the same
    // control the Who Represents Me action row carries: one address, several
    // doors, no nav pill. It is printed ONLY in the signed-in branch: signed out
    // there is no account menu to put it in, and the band's own control is the
    // way in.
    //
      ...
      el.innerHTML =
        '<div style="display:flex;align-items:center;gap:0.8rem;flex-wrap:wrap;background:linear-gradient(135deg, rgba(88,28,135,0.20) 0%, rgba(139,92,246,0.05) 100%);border:1px solid rgba(139,92,246,0.32);border-radius:1rem;padding:0.7rem 1rem;margin-bottom:1.5rem;">' +
          ...
          '<div style="flex:1;min-width:180px;">' +
            '<div class="font-condensed" style="font-weight:700;letter-spacing:0.03em;color:#d8b4fe;font-size:0.86rem;">Personalized Match is on — candidates show <span style="color:#e9d5ff;">Your Match&nbsp;%</span> where we can read them</div>' +
    ...
    window._myteamGuideGo = function(which) {
      try {
        ...
        } else { // 'pick' / 'fill' — open the next open seat in the workspace
          // The workspace is its own document (/ballot) now, and
          // pdxBallotWorkspaceOpen is the one name that answers on both: the
          // desk itself where the desk is the page, and a real navigation
          // carrying ?seat= from anywhere else. The Relevant-to-Me jump stays
          // as the fallback for a page that has neither.
          var _seat = _myteamFirstOpenOffice();
          if (typeof window.pdxBallotWorkspaceOpen === 'function') window.pdxBallotWorkspaceOpen(_seat);
          else if (window.jumpToRelevantAccordion) window.jumpToRelevantAccordion(_seat);
        }
      } catch (e) {}
    };
    ...
    function _mypolBuildGrid() {
      ...
      // The chip ships hidden with an empty label so the document never states a
      // seat count it cannot know. Reveal it only now, with a real total behind it.
      var browseSlotsChip = document.getElementById('myteam-browse-team-status');
      if (browseSlotsChip && _counts.total) browseSlotsChip.style.display = 'flex';
