# Voting-record coverage report

Generated by `scripts/vr-coverage-report.mjs`. Read-only; regenerate with `--write`.

Mappings are read from the live database and stated positions from the stance
files in this checkout, so a snapshot taken before a pending migration deploys
will still show the measures that migration maps as gaps. Measure titles are the
exception: a title curated in `db/vr-measure-identity.json` is shown even when the
live row still holds a placeholder, marked `pending` until the migration deploys.

## Where the ranking stands

| | member-votes |
|---|---|
| Recorded yea/nay votes | 5,746 |
| …on a measure with at least one issue mapping | 3,844 |
| …**and** the member holds a stated position on that key (rankable) | 1,587 |
| Blocked earlier than that — the measure has no real title yet | 0 |
| …title resolved in `db/vr-measure-identity.json`, awaiting deploy | 766 |

Rankable (member, issue) pairs: **577**. People with at least one rankable record: **182**.

## Gap 0 — measures still carrying a placeholder title

A measure the vote feed could only label "Roll call N" (or by its bare citation)
is not a mapping opportunity yet, it is an *identity* gap: which issues a bill
speaks to cannot be judged before anyone knows which bill it is. These are counted
separately from Gap 1 so an identity backfill is never mistaken for mapping work.

19 measure(s) carrying 766 member-votes have an identity in
`db/vr-measure-identity.json` and appear below under their real titles, marked
`pending` where the live row has not been updated yet.

No measure carrying a vote is left without a title.

## Gap 1 — measures with votes and no issue mapping

47 measure(s), 1,902 member-votes at stake.

| member-votes | measure | title | |
|---:|---|---|---|
| 74 | H.Res. 916 | H.Res. 916 — Rule providing for consideration of H.R. 4312, H.R. 1005, H.R. 1049, H.R. 106 |  |
| 74 | H.Res. 1423 | Providing for consideration of the bill (H.R. 139) to make daylight savings time permanent |  |
| 74 | H.Res. 1438 | Providing for consideration of the bill (H.R. 8800) to authorize appropriations for fiscal |  |
| 73 | H.Res. 682 | H.Res. 682 — Rule providing for consideration of H.R. 3838 and H.R. 3486 |  |
| 70 | H.Res. 1398 | Providing for consideration of the bill (H.R. 8800) to authorize appropriations for fiscal |  |
| 66 | H.Res. 1075 | H.Res. 1075 — Rule providing for consideration of H.R. 4626 and H.R. 4758 |  |
| 38 | H.R. 36 | H.R. 36 — MEGOBARI Act |  |
| 38 | S.J.Res. 18 | A joint resolution disapproving the rule submitted by the Bureau of Consumer Financial Pro | `pending` |
| 38 | H.R. 530 | ACES Act | `pending` |
| 38 | (no number) | Election of the Speaker |  |
| 38 | S. 1071 | National Defense Authorization Act for Fiscal Year 2026 | `pending` |
| 38 | H.R. 1402 | TICKET Act | `pending` |
| 38 | H.Amdt. 259 | H.Amdt. 259 (Issa) to H.R. 8800 — authorize the Secretary to approve any eligible investor |  |
| 38 | H.Amdt. 261 | H.Amdt. 261 (Harrigan) to H.R. 8800 — prohibit the use of automated speed enforcement came |  |
| 37 | H.J.Res. 78 | Providing for congressional disapproval under chapter 8 of title 5, United States Code, of | `pending` |
| 37 | H.Res. 377 | Providing for consideration of the bill (H.R. 276) to rename the Gulf of Mexico as the "Gu | `pending` |
| 37 | H.R. 1503 | Stop Forced Organ Harvesting Act of 2025 | `pending` |
| 37 | H.Amdt. 242 | H.Amdt. 242 (Boebert) to H.R. 8800 — strike section 1213, relating to the deadline for the |  |
| 37 | H.R. 7401 | Small Business Lending Fraud Prevention Act |  |
| 37 | H.R. 1118 | Value Over Cost Act of 2026 |  |
| 37 | H.Amdt. 243 | H.Amdt. 243 (Boebert) to H.R. 8800 — strike section 521 and replace it with a prohibition  |  |
| 37 | H.Amdt. 245 | H.Amdt. 245 (Boebert) to H.R. 8800 — strike section 518, relating to the pilot program on  |  |
| 37 | H.Amdt. 266 | H.Amdt. 266 (Grothman) to H.R. 8800 — require the Secretary of Defense to submit a report  |  |
| 37 | H.Con.Res. 113 | Establishing the congressional budget for the United States Government for fiscal year 202 |  |
| 36 | H.R. 1005 | Combating the Lies of Authoritarians in School Systems Act | `pending` |
| 36 | H.R. 1049 | Transparency in Reporting of Adversarial Contributions to Education Act | `pending` |
| 36 | H.R. 973 | Setting Consumer Standards for Lithium-Ion Batteries Act | `pending` |
| 36 | H.R. 915 | Small Business Technological Advancement Act |  |
| 36 | H.R. 2478 | Financial Exploitation Prevention Act of 2025 |  |
| 36 | H.R. 3106 | Weatherizing Infrastructure in the North and Terrorism Emergency Readiness Act of 2025 |  |
| 36 | H.R. 8897 | Improving Travel for American Families Act |  |
| 36 | H.R. 139 | Sunshine Protection Act of 2025 |  |
| 36 | H.Amdt. 244 | H.Amdt. 244 (Boebert) to H.R. 8800 — strike section 524, which removes the existing statut |  |
| 35 | H.R. 1069 | PROTECT Our Kids Act | `pending` |
| 35 | H.R. 5362 | To name the Department of Veterans Affairs multispecialty clinic in Marietta, Georgia, as  |  |
| 35 | H.R. 4541 | To reauthorize the Young Women’s Breast Health Education and Awareness Requires Learning Y |  |
| 35 | H.R. 8823 | Putting Patients First by Strengthening Provider Accountability in FECA Act |  |
| 34 | H.R. 3424 | H.R. 3424 — SPACE Act of 2025 |  |
| 34 | H.R. 3425 | H.R. 3425 — Personnel Oversight and Shift Tracking Act of 2025 |  |
| 34 | S. 2503 | ROTOR Act | `pending` |
| 34 | H.Res. 377 | Providing for consideration of the bill (H.R. 276) to rename the Gulf of Mexico as the "Gu | `pending` |
| 34 | H.R. 1676 | Make SWAPs Efficient Act of 2025 | `pending` |
| 34 | S. 356 | Secure Rural Schools Reauthorization Act of 2025 | `pending` |
| 33 | H.R. 5348 | H.R. 5348 — Social Security Child Protection Act of 2025 |  |
| 33 | H.R. 4423 | H.R. 4423 — No New Burma Funds Act |  |
| 32 | H.R. 7128 | TRIA Program Reauthorization Act of 2026 |  |
| 31 | H.R. 7757 | KIDS Act |  |

## Gap 1b — measures mapped to exactly one issue

Not necessarily wrong: a single-issue bill should have a single key. Listed so a
second, genuinely-supported key is not overlooked on a high-traffic measure.

| member-votes | measure | key |
|---:|---|---|
| 83 | PN11-7 | strong_defense |
| 83 | Kennedy — HHS | healthcare |
| 74 | H.R. 6955 | econ_smallbiz |
| 73 | H.R. 8800 | strong_defense |
| 40 | H.R. 276 | america_first |
| 38 | H.Amdt. 97 | lands_preserve |
| 38 | H.Amdt. 85 | lgbtq_rights |
| 38 | H.Amdt. 253 | gun_rights |
| 38 | H.Amdt. 257 | school_choice |
| 37 | H.Amdt. 254 | lgbtq_rights |
| 37 | H.Amdt. 256 | lgbtq_rights |
| 37 | H.Amdt. 258 | religious_liberty |
| 36 | H.Amdt. 247 | checks_balances |
| 36 | H.Amdt. 249 | states_federal_power |
| 36 | H.Amdt. 250 | states_federal_power |
| 36 | H.R. 8884 | social_security |
| 35 | H.Res. 1399 | gov_transparency |
| 35 | H.R. 9237 | veterans |
| 30 | H.R. 6329 | gov_transparency |
| 18 | S.Amdt. 3428 | gov_transparency |
| 17 | S. 2296 | strong_defense |

## Gap 2 — members with votes on an issue but no stated position

5,495 (member, issue) pairs, 11,050 member-votes.
Each row is one sourced position away from being judgeable — but only where a
real, verifiable position exists. Do not fill these in by inference.

| member-votes | member | issue key | profile exists |
|---:|---|---|---|
| 12 | scalise | strong_defense | yes |
| 12 | emmer | gov_regulation | yes |
| 11 | mike_johnson | strong_defense | yes |
| 11 | emmer | strong_defense | yes |
| 11 | aoc | strong_defense | yes |
| 11 | mike_johnson | gov_regulation | yes |
| 11 | scalise | gov_regulation | yes |
| 10 | julie_fedorchak | strong_defense | yes |
| 10 | julie_fedorchak | cut_spending | yes |
| 10 | troy_downing | strong_defense | yes |
| 10 | troy_downing | cut_spending | yes |
| 10 | mike_simpson | strong_defense | yes |
| 10 | mike_simpson | cut_spending | yes |
| 10 | mike_flood | strong_defense | yes |
| 10 | jason_smith | strong_defense | yes |
| 10 | jason_smith | cut_spending | yes |
| 10 | kclark | strong_defense | yes |
| 10 | kclark | cut_spending | yes |
| 10 | khanna | cut_spending | yes |
| 10 | jim_jordan | gov_regulation | yes |
| 10 | tlaib | strong_defense | yes |
| 10 | tlaib | cut_spending | yes |
| 10 | jayapal | strong_defense | yes |
| 10 | jayapal | cut_spending | yes |
| 10 | crockett | strong_defense | yes |
| 10 | khanna | strong_defense | yes |
| 9 | aoc | cut_spending | yes |
| 9 | jim_jordan | strong_defense | yes |
| 9 | aoc | gov_regulation | yes |
| 9 | khanna | gov_regulation | yes |
| 9 | owens | gov_regulation | yes |
| 9 | stephanie_bice | cut_spending | yes |
| 9 | tom_cole | cut_spending | yes |
| 9 | rob_bresnahan | cut_spending | yes |
| 9 | rick_crawford | cut_spending | yes |
| 9 | french_hill | cut_spending | yes |
| 9 | mike_ezell | cut_spending | yes |
| 9 | trent_kelly | cut_spending | yes |
| 9 | michael_guest | cut_spending | yes |
| 9 | maxine_waters | cut_spending | yes |
| 9 | bennie_thompson | cut_spending | yes |
| 9 | bruce_westerman | cut_spending | yes |
| 9 | frank_lucas | cut_spending | yes |
| 9 | mariannette_miller_meeks | cut_spending | yes |
| 9 | mike_collins | cut_spending | yes |
| 9 | rob_bresnahan | strong_defense | yes |
| 9 | mike_collins | strong_defense | yes |
| 9 | josh_brecheen | strong_defense | yes |
| 9 | french_hill | strong_defense | yes |
| 9 | maxine_waters | strong_defense | yes |
| 9 | michael_guest | strong_defense | yes |
| 9 | mariannette_miller_meeks | strong_defense | yes |
| 9 | steve_womack | strong_defense | yes |
| 9 | bruce_westerman | strong_defense | yes |
| 9 | owens | america_first_fp | yes |
| 9 | aoc | america_first_fp | yes |
| 9 | bennie_thompson | america_first_fp | yes |
| 9 | bruce_westerman | america_first_fp | yes |
| 9 | crockett | america_first_fp | yes |
| 9 | french_hill | america_first_fp | yes |
| … | _5,435 more_ | | |

## Gap 2 rolled up by issue

| member-votes | issue key | members missing a position |
|---:|---|---:|
| 748 | gov_regulation | 203 |
| 639 | states_federal_power | 199 |
| 619 | climate_action | 202 |
| 596 | strong_defense | 112 |
| 583 | cut_spending | 135 |
| 514 | cost_living | 206 |
| 509 | energy_production | 170 |
| 450 | america_first_fp | 113 |
| 414 | national_debt | 125 |
| 301 | gov_transparency | 112 |
| 282 | tough_on_crime | 106 |
| 267 | gov_services | 117 |
| 265 | foreign_balance | 100 |
| 261 | healthcare | 123 |
| 253 | school_choice | 157 |
| 243 | deportations | 135 |
| 226 | checks_balances | 105 |
| 194 | lgbtq_rights | 74 |
| 186 | restraint | 118 |
| 182 | border_security | 93 |
| 180 | lands_energy | 136 |
| 176 | family_support | 137 |
| 170 | lower_taxes | 131 |
| 145 | econ_smallbiz | 37 |
| 139 | free_speech | 127 |
| 135 | america_first | 116 |
| 135 | edu_college_cost | 135 |
| 131 | tax_middle_class | 131 |
| 128 | public_schools | 128 |
| 123 | privacy_rights | 75 |

