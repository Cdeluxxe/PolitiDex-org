...
// ═════════════════════════════════════════════════════════════════════════════
{
  ...
  const versions = fs.readdirSync(MIG_DIR)
    ...
    .map((f) => f.replace(/\.sql$/, ''))
    .sort();
  eq(versions[versions.length - 1], '20260910000000_vr_densify_underweighted_landmarks',
  ...
  // densified the issue mappings on two heavily-voted measures. What it guards
