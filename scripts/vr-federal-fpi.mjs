...
// database holds right now. "after" is that set OVERLAID with the wave's mapping
// seed (db/vr-federal-mapping-seed-f1.json) in memory only. Nothing is written:
...
// what makes the table quotable before the deploy rather than after it.
//
...
const SEED_FILE = "db/vr-federal-mapping-seed-f1.json";
const VOTE_SEED_FILE = "db/vr-federal-depth-vote-seed.json";
...
//
// This is a projection of what the migration writes, not a substitute for it. The
// numbers it produces are honest about one thing only: what the record WILL read
// like once 20261009000000_vr_federal_formal_depth_f1.sql is applied.
function voteOverlay(lane, pids) {
  if (!existsSync(join(ROOT, VOTE_SEED_FILE))) return { lane, addedRolls: 0, addedVotes: 0, addedMeasures: 0, skippedRolls: 0 };
  const seed = J(VOTE_SEED_FILE);
  ...
  for (const v of seed.votes || []) {
    ...
    if (liveRoll.has(rk)) { skippedRolls++; continue; }   // already in the database
...
function overlay(issues, measures) {
  if (!existsSync(join(ROOT, SEED_FILE))) return { rows: issues.slice(), added: 0, skipped: 0 };
  const seed = J(SEED_FILE);
  const byKey = new Map();
...
async function resolveSet() {
  ...
  const seed = existsSync(join(ROOT, SEED_FILE)) ? J(SEED_FILE) : { measures: [] };
  const pairs = (seed.measures || []).map((m) => [m.congress, String(m.number).trim()]);
  const touched = new Set(UT);
  // Every member the wave ATTRIBUTES, not only the ones the database already
  // holds. Three of the four measures are not in the corpus yet, so a
  // database-only definition of "touched" would silently shrink to the Utah set
  // plus whoever voted on the one live roll — and would report a national pass as
  // a Utah-sized one.
  if (existsSync(join(ROOT, VOTE_SEED_FILE))) {
    for (const v of J(VOTE_SEED_FILE).votes || []) {
...
if (argOf("row")) {
  ...
  console.log(JSON.stringify({ set: SET, seed: { added: O.added, skipped: O.skipped },
  ...
  console.log(`\n  FEDERAL FORMAL PATTERN INDEX — set "${SET}" — shipped tiers, no floor moved\n`);
