...
export function buildSql(session, opts) {
  ...
  // THE PACK STEP, DECLARED. Not a federal-only line: the pack key is a fingerprint
  // of vr_measure_issues and the table does not know which jurisdiction wrote a row,
  // so a Utah committee wave retires packs exactly the way a federal promote does.
  // scripts/test-vr-mapping-migration-pack-step.mjs fails CI on a mapping migration
  // that ships without this, and asserts this generator still emits it.
  L.push(`--`);
  L.push(`-- pack-generation: derived — up to ${c.issueMappings} vr_measure_issues row(s) below, each`);
