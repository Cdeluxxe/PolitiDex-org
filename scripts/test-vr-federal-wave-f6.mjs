...
// ── 5. the vocabulary: nothing added, nothing restuffed ─────────────────
{
  ...
  // F6 needs is that no key was added and no boundary widened, and that is
  ...
  }
  {
    ...
    if (ok(h !== null, "could not read HEAD:alignment-tool.js"))
      assertParentTableIsTheOnlyMove({ ok, eq }, h, R("alignment-tool.js"), "F6");
...
}
{
  ...
  // what is inside them is argued rather than excused. F6 has no stake in it: the span reads counts the record lane already publishes and adds a sentence naming the documents behind them.
