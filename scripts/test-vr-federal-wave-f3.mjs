...
}
{
  ...
  if (!ok(!!(head && head.PDXWordAction && head.PDXWordAction.read), "the pre-wave engine booted from HEAD")
    ...
    // scroll the rest of the report off the screen is a failure message nobody reads.
    {
      ...
      eq(added.length, 0, `F3 added ${added.length} profile(s) to the roster: ${added.slice(0, 8).join(", ")}. This wave maps measures in the DATABASE; a new pid here means a mapping leaked into a shipped file.`);
      const firstMove = PIDS.findIndex((p, n) => nowPids[n] !== p);
      ok(gone.length || added.length || firstMove < 0,
        `F3 reordered the roster — first divergence at index ${firstMove} (${PIDS[firstMove]} → ${nowPids[firstMove]})`);
    }
