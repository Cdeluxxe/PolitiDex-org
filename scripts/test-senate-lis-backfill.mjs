#!/usr/bin/env node
// ---------------------------------------------------------------------------
// PolitiDex — Senate LIS attribution backfill discipline
// ---------------------------------------------------------------------------
// The Senate twin of scripts/test-house-evs-backfill.mjs, holding the same line for
// db/vr-senate-lis-backfill-seed.json and the migration generated from it. The rules
// are the same because the risk is the same: a densification pass touches thousands of
// rows at once, every row is boring on its own, and the reader-facing effect — a
// senator's record suddenly having a shape — is exactly what a fabricated cell would
// also produce.
//
//   * It ADDS, and only adds. vr_member_votes is the only table written, every insert
//     is ON CONFLICT DO NOTHING, and there is no UPDATE, DELETE or DO UPDATE anywhere.
//     That is what makes the one contradicted stored cell safe: the migration physically
//     cannot overwrite a stored vote, so the disagreement between our record and the
//     Secretary of the Senate's stays visible instead of being laundered away.
//   * Attribution is fail-closed, and on this side it runs one hop longer than the
//     House's. The Senate's document identifies a voter by <lis_member_id>, so every
//     cell has to carry that id, the Bioguide it resolved to, and the slug that Bioguide
//     maps to today — three published identifiers, no name matching. A guessed member
//     scores the wrong person forever.
//   * The seed and the SQL tell the same story: the generator is re-run and its output
//     compared byte-for-byte with the committed migration.
//   * Every contradicted cell is absent from the insert set, and every rejected roll is
//     absent entirely. A roll the pull refused to verify must not appear in the fill.
//   * Slice 1's thresholds are untouched. More record is the only permitted way to clear
//     the coverage floor.
//
...
const SEED_REL = 'db/vr-senate-lis-backfill-seed.json';
const MIG_REL = 'netlify/database/migrations/20260908000000_vr_senate_lis_attribution_backfill.sql';
...
ok(String(seed.chamber).toLowerCase() === 'senate', 'seed is not marked as Senate');
...
ok(/senate\.gov/i.test(String(seed.source || '')), 'seed does not cite senate.gov as its source');
// Every roll the pull looked at is accounted for: filled, refused by name, or already
// complete. A shrinking denominator is how coverage claims start to flatter themselves.
ok(seed.rollsConsidered === seed.votes.length + seed.rollsRejected + seed.rollsAlreadyComplete,
  `${seed.rollsConsidered} rolls considered ≠ ${seed.votes.length} filled + ${seed.rollsRejected} rejected + ${seed.rollsAlreadyComplete} already complete`);
ok((seed.rejectedRolls?.rows || []).length === seed.rollsRejected,
  'the rejected rolls are counted but not enumerated — a refusal has to say what it refused');
ok((seed.rejectedRolls?.rows || []).every(r => Array.isArray(r.why) && r.why.length),
  'a rejected roll carries no reason');
...
for (const { v, r } of allCells) {
  ...
  // The Senate names its voters by LIS id and nothing else, so the id it was read under
  // travels with the cell. Without it there is no way back to the document.
  if (!/^S\d+$/.test(String(r.lisMemberId || ''))) badLis.push(`${where} ${r.politicianId}`);
...
ok(!badLis.length, `cell(s) with no LIS member id: ${badLis.slice(0, 4).join('; ')}`);
...
// One LIS id may not stand for two people on one roll, and one person may not arrive
// under two LIS ids — either would mean the identity hop is not a function.
const lisCollision = [];
for (const v of seed.votes) {
  const byLis = new Map();
  for (const r of v.memberVotes) {
    if (byLis.has(r.lisMemberId) && byLis.get(r.lisMemberId) !== r.politicianId) {
      lisCollision.push(`${v.congress}/${v.session} roll ${v.rollNumber} ${r.lisMemberId}`);
    }
    byLis.set(r.lisMemberId, r.politicianId);
  }
}
ok(!lisCollision.length, `one LIS id resolved to two members: ${lisCollision.slice(0, 3).join('; ')}`);
...
for (const v of seed.votes) {
  ...
  ok(/^https:\/\/www\.senate\.gov\/legislative\/LIS\/roll_call_votes\//.test(String(v.sourceUrl || '')),
    `${key} does not cite a Senate LIS document`);
  ok(Number.isInteger(v.rollNumber) && v.rollNumber > 0,
    `${key} has no stored roll number — a roll with nothing to verify against is refused, not filled`);
...
// Each is a cell we ALREADY store whose stored position disagrees with the Senate's
// document. It is not fillable — something is there — and this pass has no opinion
// about which one is right. Assert its absence rather than trusting it.
const byRoll = new Map(seed.votes.map(v => [`${v.congress}|${v.session}|${v.rollNumber}`, v]));
const leaked = [];
for (const d of seed.discrepancies?.rows || []) {
  const v = byRoll.get(`${d.congress}|${d.session}|${d.roll}`);
  if (v && v.memberVotes.some(r => r.politicianId === d.slug)) leaked.push(`${d.congress}/${d.session} roll ${d.roll} ${d.slug}`);
}
ok(!leaked.length, `contradicted stored cell(s) present in the insert set: ${leaked.join('; ')}`);
// A roll the pull refused must not come back through the front door.
const refused = [];
for (const r of seed.rejectedRolls?.rows || []) {
  if (r.roll != null && byRoll.has(`${r.congress}|${r.session}|${r.roll}`)) {
    refused.push(`${r.congress}/${r.session} roll ${r.roll}`);
  }
}
ok(!refused.length, `roll(s) rejected by the pull are being filled anyway: ${refused.join('; ')}`);
...
const regenerated = execFileSync(process.execPath, [path.join(ROOT, 'scripts', 'vr-gen-senate-lis-backfill-migration.mjs')], {
  cwd: ROOT,
  encoding: 'utf8',
...
ok(!/chamber = 'house'/i.test(sql), 'the Senate migration looks up a House roll call');
...
if (fails.length) {
  console.error(`✗ Senate LIS backfill discipline: ${fails.length} failure(s)`);
  fails.forEach(f => console.error(`  - ${f}`));
  process.exit(1);
}
console.log(`✓ Senate LIS backfill discipline: all ${pass} assertions passed`);
