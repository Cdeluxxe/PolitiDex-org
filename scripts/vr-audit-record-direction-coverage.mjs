...
const before = new Map(); // pid -> items[]
const held = new Set();   // rollcall|pid, so the union honours ON CONFLICT DO NOTHING
for (const v of live) {
  const it = item(v.rollcall_id, v.position);
  if (!it) continue;
  held.add(v.rollcall_id + "|" + v.politician_id);
  if (!before.has(v.politician_id)) before.set(v.politician_id, []);
  before.get(v.politician_id).push(it);
}
const after = new Map([...before].map(([k, v]) => [k, v.slice()]));
...
for (const rel of SEEDS) {
  ...
  for (const v of seed.votes) {
    ...
    for (const r of v.memberVotes) {
      if (held.has(id + "|" + r.politicianId)) { collided++; continue; }
      held.add(id + "|" + r.politicianId);
      if (!after.has(r.politicianId)) after.set(r.politicianId, []);
      after.get(r.politicianId).push(item(id, r.position));
      added++;
    }
  }
}
...
console.log(`seed cells: ${added} added · ${collided} already stored and left alone`);
